...
for version in "${version_array[@]}"; do
    ...
    # Step 1: Generate config from templates
    ...
    # Step 2: Generate Dockerfile and build assets
    config_file="$PROJECT_DIR/configs/generated/asterisk-$version-"*".yml"
    config_file=$(ls $config_file 2>/dev/null | head -1)

    if [[ -z "$config_file" ]]; then
        log ERROR "No config file found for $version"
        ((failed++))
        continue
    fi

    # Extract distribution from config filename
    distribution=$(basename "$config_file" .yml | sed "s/asterisk-$version-//")
    build_dir="$PROJECT_DIR/asterisk/$version-$distribution"

    # Create build directory
    mkdir -p "$build_dir"

    # Generate Dockerfile
    if python3 "$SCRIPT_DIR/generate-dockerfile.py" "$config_file" --output "$build_dir/Dockerfile" --templates-dir "$PROJECT_DIR/templates/dockerfile" >/dev/null 2>&1; then

        # Generate simple build.sh
...
EOF
        ...
        # Generate simple healthcheck.sh
        cat > "$build_dir/healthcheck.sh" << EOF
#!/bin/bash
# Healthcheck for Asterisk $version
...
EOF
        ...
        log ERROR "Dockerfile generation failed: $version"
        ((failed++))
    fi
